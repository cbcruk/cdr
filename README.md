...
데모의 "비동기 작업" 버튼이 이 경계를 그대로 보여준다. `await` 앞뒤로 한 번씩
기록하는데, 앞의 레코드에만 `trace_id`가 붙는다.
...
- **잃으면 안 되는 것은 배치에 두지 않는다**: `flushOn` 이상(기본 `error`)은
  즉시 내보낸다. `pagehide` flush는 보험이 못 된다. IndexedDB 쓰기는
  비동기라 페이지가 헐리는 시점에 시작하면 끝나지 않는다. e2e로 확인한
  사실이고, 그래서 심각한 레코드는 애초에 버퍼를 건너뛴다. 그 아래 레벨은
  탭이 갑자기 닫히면 사라질 수 있다.
...
pnpm demo     # 데모 실행 (http://localhost:5173)
```

### 데모

`pnpm demo`로 [demo/](demo/)를 띄우면 pull 모델을 직접 만져볼 수 있다. 한 페이지에
두 탭:
